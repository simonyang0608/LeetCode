class Solution(object):
    def minimumEffortPath(self, heights):
        ...
        record_min_val, record_max_val = ((10 ** 6) + 1), 0
        ...
        for rows_idx in range(len_rows):
            for cols_idx in range(len_cols):
                record_min_val = ((heights[rows_idx])[cols_idx] if ((heights[rows_idx])[cols_idx] < record_min_val) else record_min_val) #Keep updating/overwriting
                record_max_val = ((heights[rows_idx])[cols_idx] if ((heights[rows_idx])[cols_idx] > record_max_val) else record_max_val) #Keep updating/overwriting
        ...
        while (record_left_ptr < record_right_ptr):
            ...
            record_flag = self.isValid(len_rows, len_cols, heights, record_mid_ptr) #Record flag
    ...
    def isValid(self, len_rows, len_cols, heights, record_val):
        """
        :type len_rows: int
        :type len_cols: int
        :type heights: List[List[int]]
        ...
        while (record_queue):
            ...
            for queue_idx in range(len_queue):
                ...
                for direc_idx in range(4):
                    ...
                    if (abs(((heights[record_rows_idx])[record_cols_idx] - (heights[next_rows_idx])[next_cols_idx])) > record_val):
                        continue
