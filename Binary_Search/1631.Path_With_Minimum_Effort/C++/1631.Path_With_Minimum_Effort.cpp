...
public:
    int minimumEffortPath(vector<vector<int>> & heights) 
    {
        ...
        int record_min_val = (1e6 + 1), record_max_val = 0;
        ...
        for (int rows_idx = 0; (rows_idx < len_rows); (rows_idx++)) //Rows
        {
            ...
            {
                record_max_val = ((heights[rows_idx])[cols_idx] > record_max_val) ? (heights[rows_idx])[cols_idx] : record_max_val; //Keep updating, overwriting
                record_min_val = ((heights[rows_idx])[cols_idx] < record_min_val) ? (heights[rows_idx])[cols_idx] : record_min_val; //Keep updating, overwriting
        ...
        while (record_left_ptr < record_right_ptr) //Binary-search
        {
            ...
            bool record_flag = isValid(len_rows, len_cols, heights, record_mid_ptr); //Record flag
    ...
    bool isValid(const int & len_rows, const int & len_cols, const vector<vector<int>> & heights, 
                 const int & record_val)
    {
        ...
        //   - const vector<vector<int>> ref. (heights vector) //
        ...
        while (! record_queue.empty()) //Cycled-times (i.e. BFS)
        {
            for (int queue_idx = record_queue.size(); (queue_idx > 0); (queue_idx--)) //Queue-size
            {
                ...
                for (int direc_idx = 0; (direc_idx < 4); (direc_idx++)) //4-Sider directions
                {
                    ...
                    if (abs(((heights[record_rows_idx])[record_cols_idx] - (heights[next_rows_idx])[next_cols_idx])) > record_val) { continue; }
