class Solution(object):
    def minDistance(self, word1, word2):
        ...
        ##### Step 2: Looped-traversal with dynamic-programming array #####
        for rows_idx in range(1, (len_word1 + 1)):
            for cols_idx in range(1, (len_word2 + 1)):

                ##### Check if the current indexed-char the same or not #####
                if (word1[(rows_idx - 1)] == word2[(cols_idx - 1)]):
                    ...
                    (record_dp[rows_idx])[cols_idx] = (min((record_dp[(rows_idx - 1)])[cols_idx], (record_dp[rows_idx])[(cols_idx - 1)]) + 1) #Keep updating/overwriting
