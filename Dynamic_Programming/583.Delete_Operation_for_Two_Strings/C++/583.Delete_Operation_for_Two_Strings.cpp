...
public:
    int minDistance(string word1, string word2) 
    {
        ...
        ///// Step 1: Record rows, columns information with dynamic-programming vector /////
        ...
        for (int rows_idx = 1; (rows_idx <= len_word1); (rows_idx++)) //Rows
        {
            for (int cols_idx = 1; (cols_idx <= len_word2); (cols_idx++)) //Columns
            {
                ...
                else { (record_dp[rows_idx])[cols_idx] = (min((record_dp[(rows_idx - 1)])[cols_idx], (record_dp[rows_idx])[(cols_idx - 1)]) + 1); } //Keep updating, overwriting
